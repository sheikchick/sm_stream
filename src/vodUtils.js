...
async function main() {
    try {
        if (process.argv.length < 4) {
            console.log("USAGE: node vodutils.js [vods | thumbnails] [tournament-name] [webpage_url]")
            process.exit()
        }

        switch (process.argv[2]) {
            case "vodred":
                console.log("Creating VODs:")
                sets = JSON.parse(fs.readFileSync(path.join(process.cwd(), "../data/json/tournaments", process.argv[3], "tournament-sets.json"), 'utf8'));
                sets.forEach((set) => {
                    recordLive.createVod(set, process.argv[3].replaceAll("_", " "))
                })
                break;
            case "vods":
                console.log("Creating VOD commands:")
                createVods(path.join(process.cwd(), "../data/json/tournaments", process.argv[3], "tournament-sets.json"), process.argv[3]);
                break;
            case "thumbnails":
