...
function reset_background(player) {
	$(`#${player}_characters .css-character`).css("background-color", "transparent");
}
...
function load_char_actual(player, character, colour) {
	...
	characterActual.attr("src", `/stock?${load_char_string(character, colour)}`);
}

function load_char_change(player, character, colour) {
	const characterChange = $(`#${player}_character_change`);
	characterChange.attr("src", `/csp?${load_char_string(character, colour)}`);
