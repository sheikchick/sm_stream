...
exports.createVod = (data, tournamentName) => {
    try {
        ...
        setName = `${replaceIllegal(p1Name).trim()} vs ${replaceIllegal(p2Name).trim()} – ${replaceIllegal(tournamentName).trim()} – ${data.round.startsWith("Pools") ? "Pools" : replaceIllegal(data.round).trim()}.mp4`
